...
public static class SpriteSheetProcessor
{
    ...
    /// </returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="file"/> is <see langword="null"/>.</exception>
    public static SpriteSheet Process(AsepriteFile file, ICollection<AsepriteLayer> layers, bool mergeDuplicateFrames = true, int borderPadding = 0, int spacing = 0, int innerPadding = 0)
    {
        ArgumentNullException.ThrowIfNull(file);
